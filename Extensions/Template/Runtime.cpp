...
//
//	if (Vox::GetInstance().surface == nullptr) {
//		LPSURFACE proto = NULL;
//		if (windowSurface != NULL) {
//			GetSurfacePrototype(&proto, windowSurface->GetDepth(), ST_MEMORYWITHDC, SD_DIB);
//		}
//		Vox::GetInstance().surface = NewSurface();
//		Vox::GetInstance().surface->Create(width, height, proto);
//		Vox::GetInstance().surface->CreateAlpha();
//	}
//
//	Vox::GetInstance().Draw(x, y, width, height);
//	
//	return Vox::GetInstance().surface;
//}
...
LPSMASK MMF2Func GetRunObjectCollisionMask(RD *rd, LPARAM lParam)
{
	...
	//if ( (rd->rs->rsEffect & EFFECTFLAG_TRANSPARENT) == 0 )	// Note: only if your object has the OEPREFS_INKEFFECTS option
	//	return NULL;
	//
	//// Transparent? Create mask
	//if (rd->pExtension->vox->mask == nullptr )
	//{
	//	if ( rd->pExtension->vox->surface != NULL )
	//	{
	//		DWORD dwMaskSize = rd->pExtension->vox->surface->CreateMask(NULL, lParam);
	//		if ( dwMaskSize != 0 )
	//		{
	//			LPSMASK mask = (LPSMASK)calloc(dwMaskSize, 1);
	//			if (mask != NULL )
	//			{
	//				rd->pExtension->vox->surface->CreateMask(mask, lParam);
	//				rd->pExtension->vox->mask = &mask;
	//			}
	//		}
	//	}
	//}
	...
	//
	//return LPSMASK(rd->pExtension->vox->mask);
}
